...
The [SafeKit virtual IP address](</safekit-docs/best-practises/how-a-virtual-ip-address-works/>) configuration does not require any special network configuration and the network load balancing can run in any environment. An important feature when the solution must be deployed in an unknown infrastructure: unknown switches or routers, physical servers or virtual servers.
