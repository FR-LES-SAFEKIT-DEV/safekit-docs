...
lang: "en"
topics: "High Availability Quick Installation Guide for Firebird, SafeKit High Availability Limitations, Overview of the SafeKit / Firebird solution, Installation of the SafeKit / Firebird solution on Windows and Linux (firebird.safe), Step by step configuration of the SafeKit / Firebird solution, Demonstration of the SafeKit mirror solution, 🔍 SafeKit High Availability Navigation Hub"
---

# Firebird High Availability with SafeKit: Install the firebird.safe Module for Failover


<!-- BEGIN INSERT: insert-safekit-mirror-guide-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 

## High Availability Quick Installation Guide for Firebird {#quick-guide}

This guide explains how to set up a **mirror cluster** for Firebird using SafeKit, ensuring automatic failover and synchronous replication without the need for shared storage. 
...
### 2\. Installation
  ...
  * **Module:** Download the pre-configured `firebird.safe` application module.


### 3\. Configuration

  * **Web Console:** Configure the specific folders containing the Firebird files.
  * **Monitoring:** Start monitoring and protecting the Firebird application.
...
<!-- END INSERT: insert-safekit-mirror-guide-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 

## Overview of the SafeKit / Firebird solution

The solution is described here: **[The Simplest Firebird High Availability: 2-Node Synchronous Replication& Failover](</safekit-docs/solutions/firebird-high-availability-synchronous-replication-failover/>)**.

## Installation of the SafeKit / Firebird solution on Windows and Linux (firebird.safe)
...
<!-- BEGIN INSERT: insert-safekit-installation-package-windows-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
...
<!-- END INSERT: insert-safekit-installation-package-windows-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 


<!-- BEGIN INSERT: insert-safekit-82-installation-module-windows-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 

### Module installation on Windows

  * Download the firebird.safe module. The module is free. It contains the files userconfig.xml and the restart scripts.
  * Put **firebird.safe** under **C:/safekit/Application_Modules/generic**.


<a class="btn-download" href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/modules_windows/firebird.safe" download="firebird.safe">⬇️ Download firebird.safe (Windows) ></a> 


<!-- END INSERT: insert-safekit-82-installation-module-windows-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 


<!-- BEGIN INSERT: insert-safekit-installation-package-linux-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
...
<!-- END INSERT: insert-safekit-installation-package-linux-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 


<!-- BEGIN INSERT: insert-safekit-82-installation-module-linux-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 

### Module installation on Linux

  * Download the firebird.safe module.   
The module is free. It contains the files userconfig.xml and the restart scripts. 
  * Put **firebird.safe** under **/opt/safekit/Application_Modules/generic/**.


<a class="btn-download" href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/modules_linux/firebird.safe" download="firebird.safe">⬇️ Download firebird.safe (Linux) ></a> 


<!-- END INSERT: insert-safekit-82-installation-module-linux-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 

## Step by step configuration of the SafeKit / Firebird solution


<!-- BEGIN INSERT: insert-safekit-82-configuration-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
 

### Prerequisites

  * Firebird application installed on 2 nodes.
...
<img src="/safekit-docs/wp-content/uploads/2024/03/01-login-large.png" alt="Start the SafeKit web console to configure the Firebird cluster">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/02-cluster-edit.png" alt="Enter the nodes of the Firebird cluster">


### 3\. Select a module

  * In `New module`, click on the `firebird.safe` module.
...
<img src="/safekit-docs/wp-content/uploads/2024/03/04-module-new.png" alt="Choose the module for Firebird">
...
<div class="callout"><img src="/safekit-docs/wp-content/uploads/2022/07/note.png" alt="Note"><span>In a cloud deployment, you do not need to configure a virtual IP address in SafeKit. The virtual IP is managed at the <a href="/safekit-docs/resources/safekit-user-guide-82/#Cloud">cloud load balancer level</a>.</span></div>
...
<img src="/safekit-docs/wp-content/uploads/2025/05/05-module-mirror-edit-config-3.png" alt="Enter the Firebird module settings">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/28-module-sqlserver-edit-script-3.png" alt="Enter the Firebird module settings">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/07-module-encryption-1.png" alt="Communication encryption of the Firebird module">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/08-module-save-1.png" alt="Save and apply the Firebird module configuration">
...
<div class="callout callout-warning"><img src="/safekit-docs/wp-content/uploads/2022/07/warning-small.png" alt="Warning"><span>On Linux, you may get an error at this step if the replicated directories are mount points. <a href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/knowledge-base.pdf">See SK-0030 to solve the problem</a>.</span></div>


<img src="/safekit-docs/wp-content/uploads/2025/05/09-module-save-check-1.png" alt="Check the Firebird module configuration success">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/10-monitoring-mirror-stop-stop-prim.png" alt="Start as primary the Firebird node with the up-to-date data">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/12-monitoring-alone-stop.png" alt="The first Firebird node starts as primary and becomes ALONE">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/13-monitoring-alone-stop-start.png" alt="Start the Firebird node 2">


### 12\. Verify that the cluster is operational

  * Check that the cluster is green/green with Firebird services running on the PRIM node and not running on the SECOND node.


Only changes inside files are replicated in real time in this state.


<div class="callout callout-warning"><img src="/safekit-docs/wp-content/uploads/2022/07/warning-small.png" alt="Warning"><span>Components that are clients of Firebird services must be configured with the virtual IP address. The configuration can be done with a DNS name (if a DNS name has been created and associated with the virtual IP address).</span></div>


<img src="/safekit-docs/wp-content/uploads/2024/03/14-monitoring-prim-second.png" alt="The Firebird node 2 is SECOND (green)">
...
### 13\. Testing
  ...
  * And with Microsoft Management Console (MMC) on Windows or with command lines on Linux, check the failover of Firebird services (stopped on node 1 in the `stop_prim` script and started on node 2 in the `start_prim` script).
...
<img src="/safekit-docs/wp-content/uploads/2024/03/16-monitoring-prim-second-stop.png" alt="Stop the Firebird module on the PRIM server">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/30-snapshots-mirror.png" alt="Take the Firebird snaphots for support">



<!-- END INSERT: insert-safekit-82-configuration-en lang="en" display="content" querystring="app=Firebird&amp;mod=firebird" -->
