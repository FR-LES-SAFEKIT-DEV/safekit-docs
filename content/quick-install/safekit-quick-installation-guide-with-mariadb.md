...
<!-- BEGIN INSERT: insert-safekit-mirror-guide-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 

## High Availability Quick Installation Guide for MariaDB {#quick-guide}

This guide explains how to set up a **mirror cluster** for MariaDB using SafeKit, ensuring automatic failover and synchronous replication without the need for shared storage. 
...
### 3\. Configuration
  ...
  * **Web Console:** Configure the specific folders containing the MariaDB files.
  * **Monitoring:** Start monitoring and protecting the MariaDB application.
...
<!-- END INSERT: insert-safekit-mirror-guide-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 

## Overview of the SafeKit / MariaDB solution

The solution is described here: **[The Simplest MariaDB High Availability: 2-Node Synchronous Replication& Failover](</safekit-docs/solutions/mariadb-simplest-high-availability-cluster-synchronous-replication-failover-redundant-servers/>)**.
...
<!-- BEGIN INSERT: insert-safekit-installation-package-windows-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
...
<!-- END INSERT: insert-safekit-installation-package-windows-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 


<!-- BEGIN INSERT: insert-safekit-82-installation-module-windows-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
...
### Module installation on Windows
  ...
  * Put **mysql.safe** under **C:/safekit/Application_Modules/generic**.


<a class="btn-download" href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/modules_windows/mysql.safe" download="mysql.safe">⬇️ Download mysql.safe (Windows) ></a> 


<!-- END INSERT: insert-safekit-82-installation-module-windows-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 


<!-- BEGIN INSERT: insert-safekit-installation-package-linux-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
...
<!-- END INSERT: insert-safekit-installation-package-linux-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 


<!-- BEGIN INSERT: insert-safekit-82-installation-module-linux-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
...
The module is free. It contains the files userconfig.xml and the restart scripts. 
  * Put **mysql.safe** under **/opt/safekit/Application_Modules/generic/**.


<a class="btn-download" href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/modules_linux/mysql.safe" download="mysql.safe">⬇️ Download mysql.safe (Linux) ></a> 


<!-- END INSERT: insert-safekit-82-installation-module-linux-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 

## Step by step configuration of the SafeKit / MariaDB solution


<!-- BEGIN INSERT: insert-safekit-82-configuration-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
 

### Prerequisites

  * MariaDB application installed on 2 nodes.
...
<img src="/safekit-docs/wp-content/uploads/2024/03/01-login-large.png" alt="Start the SafeKit web console to configure the MariaDB cluster">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/02-cluster-edit.png" alt="Enter the nodes of the MariaDB cluster">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/04-module-new.png" alt="Choose the module for MariaDB">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/05-module-mirror-edit-config-3.png" alt="Enter the MariaDB module settings">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/28-module-sqlserver-edit-script-3.png" alt="Enter the MariaDB module settings">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/07-module-encryption-1.png" alt="Communication encryption of the MariaDB module">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/08-module-save-1.png" alt="Save and apply the MariaDB module configuration">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/09-module-save-check-1.png" alt="Check the MariaDB module configuration success">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/10-monitoring-mirror-stop-stop-prim.png" alt="Start as primary the MariaDB node with the up-to-date data">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/12-monitoring-alone-stop.png" alt="The first MariaDB node starts as primary and becomes ALONE">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/13-monitoring-alone-stop-start.png" alt="Start the MariaDB node 2">


### 12\. Verify that the cluster is operational

  * Check that the cluster is green/green with MariaDB services running on the PRIM node and not running on the SECOND node.


Only changes inside files are replicated in real time in this state.


<div class="callout callout-warning"><img src="/safekit-docs/wp-content/uploads/2022/07/warning-small.png" alt="Warning"><span>Components that are clients of MariaDB services must be configured with the virtual IP address. The configuration can be done with a DNS name (if a DNS name has been created and associated with the virtual IP address).</span></div>


<img src="/safekit-docs/wp-content/uploads/2024/03/14-monitoring-prim-second.png" alt="The MariaDB node 2 is SECOND (green)">
...
### 13\. Testing
  ...
  * And with Microsoft Management Console (MMC) on Windows or with command lines on Linux, check the failover of MariaDB services (stopped on node 1 in the `stop_prim` script and started on node 2 in the `start_prim` script).
...
<img src="/safekit-docs/wp-content/uploads/2024/03/16-monitoring-prim-second-stop.png" alt="Stop the MariaDB module on the PRIM server">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/30-snapshots-mirror.png" alt="Take the MariaDB snaphots for support">



<!-- END INSERT: insert-safekit-82-configuration-en lang="en" display="content" querystring="app=MariaDB&amp;mod=mysql" -->
