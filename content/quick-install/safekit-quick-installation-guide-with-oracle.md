...
lang: "en"
topics: "High Availability Quick Installation Guide for Oracle, SafeKit High Availability Limitations, Overview of the SafeKit / Oracle solution, Installation of the SafeKit / Oracle solution on Windows and Linux (oracle.safe), Step by step configuration of the SafeKit / Oracle solution, Demonstration of the SafeKit mirror solution, 🔍 SafeKit High Availability Navigation Hub"
---

# Oracle High Availability with SafeKit: Install the oracle.safe Module for Failover


<!-- BEGIN INSERT: insert-safekit-mirror-guide-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 

## High Availability Quick Installation Guide for Oracle {#quick-guide}

This guide explains how to set up a **mirror cluster** for Oracle using SafeKit, ensuring automatic failover and synchronous replication without the need for shared storage. 
...
### 2\. Installation
  ...
  * **Module:** Download the pre-configured `oracle.safe` application module.


### 3\. Configuration

  * **Web Console:** Configure the specific folders containing the Oracle files.
  * **Monitoring:** Start monitoring and protecting the Oracle application.
...
<!-- END INSERT: insert-safekit-mirror-guide-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 

## Overview of the SafeKit / Oracle solution

The solution is described here: **[The Simplest Oracle High Availability: 2-Node Synchronous Replication& Failover](</safekit-docs/solutions/oracle-high-availability-synchronous-replication-failover/>)**.

## Installation of the SafeKit / Oracle solution on Windows and Linux (oracle.safe)

### Prerequisites

  * You need Oracle installed on 2 nodes (virtual machines or physical servers).



<!-- BEGIN INSERT: insert-safekit-installation-package-windows-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
...
<!-- END INSERT: insert-safekit-installation-package-windows-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 


<!-- BEGIN INSERT: insert-safekit-82-installation-module-windows-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 

### Module installation on Windows

  * Download the oracle.safe module. The module is free. It contains the files userconfig.xml and the restart scripts.
  * Put **oracle.safe** under **C:/safekit/Application_Modules/generic**.


<a class="btn-download" href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/modules_windows/oracle.safe" download="oracle.safe">⬇️ Download oracle.safe (Windows) ></a> 


<!-- END INSERT: insert-safekit-82-installation-module-windows-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 


<!-- BEGIN INSERT: insert-safekit-installation-package-linux-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
...
<!-- END INSERT: insert-safekit-installation-package-linux-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 


<!-- BEGIN INSERT: insert-safekit-82-installation-module-linux-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 

### Module installation on Linux

  * Download the oracle.safe module.   
The module is free. It contains the files userconfig.xml and the restart scripts. 
  * Put **oracle.safe** under **/opt/safekit/Application_Modules/generic/**.


<a class="btn-download" href="/safekit-docs/wp-content/uploads/downloads_safekit/version-82/modules_linux/oracle.safe" download="oracle.safe">⬇️ Download oracle.safe (Linux) ></a> 


<!-- END INSERT: insert-safekit-82-installation-module-linux-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 

## Step by step configuration of the SafeKit / Oracle solution


<!-- BEGIN INSERT: insert-safekit-82-configuration-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
 

### Prerequisites

  * Oracle application installed on 2 nodes.
...
<img src="/safekit-docs/wp-content/uploads/2024/03/01-login-large.png" alt="Start the SafeKit web console to configure the Oracle cluster">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/02-cluster-edit.png" alt="Enter the nodes of the Oracle cluster">


### 3\. Select a module

  * In `New module`, click on the `oracle.safe` module.
...
<img src="/safekit-docs/wp-content/uploads/2024/03/04-module-new.png" alt="Choose the module for Oracle">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/05-module-mirror-edit-config-3.png" alt="Enter the Oracle module settings">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/28-module-sqlserver-edit-script-3.png" alt="Enter the Oracle module settings">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/07-module-encryption-1.png" alt="Communication encryption of the Oracle module">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/08-module-save-1.png" alt="Save and apply the Oracle module configuration">
...
<img src="/safekit-docs/wp-content/uploads/2025/05/09-module-save-check-1.png" alt="Check the Oracle module configuration success">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/10-monitoring-mirror-stop-stop-prim.png" alt="Start as primary the Oracle node with the up-to-date data">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/12-monitoring-alone-stop.png" alt="The first Oracle node starts as primary and becomes ALONE">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/13-monitoring-alone-stop-start.png" alt="Start the Oracle node 2">


### 12\. Verify that the cluster is operational

  * Check that the cluster is green/green with Oracle services running on the PRIM node and not running on the SECOND node.


Only changes inside files are replicated in real time in this state.


<div class="callout callout-warning"><img src="/safekit-docs/wp-content/uploads/2022/07/warning-small.png" alt="Warning"><span>Components that are clients of Oracle services must be configured with the virtual IP address. The configuration can be done with a DNS name (if a DNS name has been created and associated with the virtual IP address).</span></div>


<img src="/safekit-docs/wp-content/uploads/2024/03/14-monitoring-prim-second.png" alt="The Oracle node 2 is SECOND (green)">
...
### 13\. Testing
  ...
  * And with Microsoft Management Console (MMC) on Windows or with command lines on Linux, check the failover of Oracle services (stopped on node 1 in the `stop_prim` script and started on node 2 in the `start_prim` script).
...
<img src="/safekit-docs/wp-content/uploads/2024/03/16-monitoring-prim-second-stop.png" alt="Stop the Oracle module on the PRIM server">
...
<img src="/safekit-docs/wp-content/uploads/2024/03/30-snapshots-mirror.png" alt="Take the Oracle snaphots for support">



<!-- END INSERT: insert-safekit-82-configuration-en lang="en" display="content" querystring="app=Oracle&amp;mod=oracle" -->
