...
description: "SafeKit High Availability for Hanwha Vision: SANless Building Management Redundancy"
category: "solutions"
lang: "en"
topics: "SafeKit for Hanwha Vision: 2-Node SANless Hyper-V Cluster, Table of Contents, How the SafeKit software simply implements a Hanwha Vision high availability cluster without shared storage on a SAN?, How the SafeKit Hanwha Vision cluster works?, SafeKit High Availability Limitations, Comparison: SafeKit for Hanwha Vision vs. Native Hyper-V Clustering, Video Guide: Configuring a SafeKit HA cluster for Hanwha Vision, SafeKit, a recognized solution on the security market, 🔍 SafeKit High Availability Navigation Hub"
---

# SafeKit High Availability for Hanwha Vision: SANless Building Management Redundancy


<!-- BEGIN INSERT: insert-safekit-hyperv-solution-mirror-en lang="en" display="content" querystring="app=Hanwha Vision" -->
 

## SafeKit for Hanwha Vision: 2-Node SANless Hyper-V Cluster {#safekit-hyperv-tco}

**SafeKit for Hanwha Vision** delivers seamless **High Availability** while significantly reducing TCO (Total Cost of Ownership). By eliminating the hardware and operational overhead of traditional clustering, SafeKit offers a lean, 2-node HA alternative to complex virtualization stacks.
...
<img src="/safekit-docs/wp-content/uploads/2026/01/safekit-vm-repli-720-720.jpg" width="512" alt="SafeKit High Availability Hanwha Vision Cluster Architecture">

**SafeKit HA Cluster for Hanwha Vision**
...
## Table of Contents {#toc}
  ...
  * [How the SafeKit software simply implements a Hanwha Vision high availability cluster without shared storage on a SAN?](<#safekit-hyperv-ha-cluster>)
  * [How the SafeKit Hanwha Vision cluster works?](<#safekit-hyperv-ha-how-it-works>)
  * [SafeKit High Availability Limitations](<#safekit-ha-limitations>)
  * [Comparison: SafeKit for Hanwha Vision vs. Native Hyper-V Clustering](<#safekit-hyperv-ha-comparison>)
  * [SafeKit video guide for Hanwha Vision](<#videoguide-safekit-hyperv-cluster>)
  * [🔍 SafeKit High Availability Navigation Hub](<#hub>)


## How the SafeKit software simply implements a Hanwha Vision high availability cluster without shared storage on a SAN? {#safekit-hyperv-ha-cluster}

### What is the SafeKit HA solution for Hanwha Vision?

SafeKit brings high availability to Hanwha Vision between two servers of any brand.

This article explains how to implement quickly a Hanwha Vision cluster without shared storage on a SAN and without specific skills.
...
### How can I run multiple applications in a high-availability Hanwha Vision cluster without a SAN?
...
SafeKit differentiates itself from traditional Hyper-V clustering solutions through its **shared-nothing architecture** and simplified management. While most competitors require a central SAN or vSAN to manage VM failover, SafeKit provides:
  ...
  * **Integrated Real-Time Replication:** Data synchronization is built directly into the solution, ensuring that Hanwha Vision virtual machines are mirrored synchronously without the overhead of managing a separate storage layer.
...
### Is it possible to set up a Hanwha Vision cluster without SAN or complex clustering skills?

Yes. This article explains how to [**quickly implement a Hyper-V cluster**](</safekit-docs/quick-install/safekit-quick-installation-guide-with-hyper-v/>) without the need for specialized SAN management or complex HA clustering skills. By using SafeKit’s **automated failover scripts** to handle the replication and restart of **Hanwha Vision** virtual machines, you get a robust redundancy solution that is significantly simpler to deploy and maintain than traditional enterprise virtualization stacks.

### Beyond Hanwha Vision, which applications and environments can SafeKit protect?
...
The following steps are described for one Hanwha Vision virtual machine inside one mirror module. Each replicated virtual machine runs in an independent mirror module (with a maximum of 32 virtual machines) with a primary server that can be either the Hyper-V server 1 or the Hyper-V server 2.

### 1\. How does real-time replication work for Hanwha Vision?

Server 1 (PRIM) runs the Hanwha Vision VM (virtual machine). SafeKit replicates in real time the VM files (virtual hard disk, VM configuration). Only changes made in the files are replicated across the network.


<img src="/safekit-docs/wp-content/uploads/2023/02/safekit-prim-second-300.png" width="300" alt="File replication at byte level in a mirror Hanwha Vision cluster">
...
### 2\. What happens during an automatic Hanwha Vision failover?

When Server 1 fails, Server 2 takes over. SafeKit restarts the Hanwha Vision VM on Server 2. Hyper-V finds the files replicated by SafeKit uptodate on Server 2.  
The VM continues to run on Server 2 by locally modifying its files that are no longer replicated to Server 1.


<img src="/safekit-docs/wp-content/uploads/2023/02/safekit-stop-alone-300.png" width="300" alt="Failover of Hanwha Vision in a mirror cluster">


The failover time is equal to the fault-detection time (set to 30 seconds by default) plus the VM reboot time.

### 3\. How is data synchronized after a Hanwha Vision failure?
...
<img src="/safekit-docs/wp-content/uploads/2023/02/safekit-resynchro-alone-300.png" width="300" alt="Failback in a mirror Hanwha Vision cluster">


Failback takes place without disturbing the Hanwha Vision VM, which can continue running on Server 2.

### 4\. How is normal Hanwha Vision operation restored?

After reintegration, the VM files are once again in mirror mode, as in step 1. The system is back in high-availability mode, with the Hanwha Vision VM running on Server 2 and SafeKit replicating file updates to Server 1.


<img src="/safekit-docs/wp-content/uploads/2023/02/safekit-second-prim-300.png" width="300" alt="Return to normal operation in a mirror Hanwha Vision cluster">
...
### How to configure a SafeKit Hanwha Vision Cluster? {#safekit-mirror-cluster-configure}


<img src="/safekit-docs/wp-content/uploads/2024/03/19-module-hyperv-edit-config-1.png" width="694" alt="SafeKit Web Console: 2-node Hanwha Vision cluster dashboard for VM replication and automatic failover.">


The **SafeKit web console** provides an intuitive interface to configure high availability of your critical applications. In just a few steps, you can configure a **SafeKit Hanwha Vision cluster** to ensure business continuity:

  * Put in `VM_PATH`, the root path of the replicated directory (D:/Repli-Hyper-V) where the Hanwha Vision VM files reside.
  * Enter in `VM_NAME`, the name of the Hanwha Vision virtual machine (VM1).
...
### How to monitor a SafeKit Hanwha Vision cluster? {#safekit-mirror-cluster-monitoring}


<img src="/safekit-docs/wp-content/uploads/2024/03/14-monitoring-prim-second.png" width="604" alt="SafeKit Web Console: Real-time monitoring of a 2-node mirror Hanwha Vision cluster showing PRIM and SECOND states with active data replication.">
...
For a 2-node Hanwha Vision cluster, the console clearly displays the roles of each server:
...
Feature | SafeKit for Hanwha Vision | MS Failover Clustering | Hyper-V Replica  
---|---|---|---  
...
<!-- END INSERT: insert-safekit-hyperv-solution-mirror-en lang="en" display="content" querystring="app=Hanwha Vision" -->
