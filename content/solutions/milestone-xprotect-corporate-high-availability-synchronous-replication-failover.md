---
title: "The Simplest Milestone XProtect High Availability: 2-Node Synchronous Replication & Failover"
slug: "milestone-xprotect-corporate-high-availability-synchronous-replication-failover"
canonical: "https://safekit.eviden.com/solutions/milestone-xprotect-corporate-high-availability-synchronous-replication-failover/"
description: "The Simplest Milestone XProtect High Availability: 2-Node Synchronous Replication & Failover"
category: "solutions"
lang: "en"
topics: "How the SANless SafeKit Mirror Cluster Works with Milestone XProtect, Table of Contents, How the SafeKit software simply implements a SANless Milestone XProtect high availability cluster?, How the SafeKit mirror cluster works with Milestone XProtect?, SafeKit High Availability Limitations, Comparison: SafeKit for Milestone XProtect vs. Other Clustering Solutions, Video Guide: Configuring a SafeKit HA mirror cluster, SafeKit, a recognized solution on the security market, 🔍 SafeKit High Availability Navigation Hub"
---
...
## How the SANless SafeKit Mirror Cluster Works with Milestone XProtect {#safekit-hyperv-tco}

**SafeKit for Milestone XProtect** delivers seamless **High Availability** while significantly reducing TCO (Total Cost of Ownership). By eliminating the hardware and operational overhead of traditional clustering, SafeKit offers a lean, 2-node HA alternative to complex clustering solutions.
...
## Table of Contents {#toc}
  ...
  * [How the SafeKit mirror cluster works with Milestone XProtect?](<#safekit-mirror-cluster>)
  * [SafeKit High Availability Limitations](<#safekit-ha-limitations>)
  * [Comparison: SafeKit for Milestone XProtect vs. Other Clustering Solutions](<#safekit-mirror-ha-comparison>)
...
### What is the SafeKit Mirror HA solution for Milestone XProtect?

SafeKit brings high availability to Milestone XProtect between two servers of any brand.

This article explains how to implement quickly a Milestone XProtect cluster without shared storage on a SAN and without specific skills.

The principle of the solution is to define the folders where the **Milestone XProtect application** data resides, its services, a virtual IP, and checkers.
...
This eliminates the "house of cards" risk where OS updates break fragile links between disparate tools, provides a single point of accountability for the entire HA stack, and reduces human error by providing a single interface for Milestone XProtect HA.
...
### What are the distinctive advantages of SafeKit Milestone XProtect high availability compared to competitors?

SafeKit differentiates itself from traditional Milestone XProtect clusters through its **shared-nothing architecture** and simplified deployment. While most enterprise solutions require complex replication deployment or expensive SAN storage, SafeKit provides:
...
### How does SafeKit reduce the TCO (Total Cost of Ownership) for Milestone XProtect compared to standard clustering?
...
Unlike traditional high-availability solutions, SafeKit is designed to operate with the **absolute minimum infrastructure overhead** , without compromising reliability. Key savings compared to traditional failover cluster include:
  ...
  * **CPU-Independent Pricing:** Licensing is independent of the number of CPUs or cores. With just two licenses for two nodes, you can protect Milestone XProtect against failures.
...
When Server 1 fails, Server 2 takes over. SafeKit switches the virtual IP address and restarts the Milestone XProtect application automatically on Server 2.
...
Feature | SafeKit for Milestone XProtect | Shared Storage Failover Clustering | Database Replication  
---|---|---|---  
