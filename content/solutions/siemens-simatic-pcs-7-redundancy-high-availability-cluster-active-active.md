...
description: "High Availability for Siemens SIMATIC PCS 7: 2-Node SANless Cluster"
category: "solutions"
lang: "en"
topics: "SafeKit for Siemens SIMATIC PCS7: 2-Node SANless Hyper-V Cluster, Table of Contents, How the SafeKit software simply implements a Siemens SIMATIC PCS7 high availability cluster without shared storage on a SAN?, How the SafeKit Siemens SIMATIC PCS7 cluster works?, SafeKit High Availability Limitations, Comparison: SafeKit for Siemens SIMATIC PCS7 vs. Native Hyper-V Clustering, Video Guide: Configuring a SafeKit HA cluster for Siemens SIMATIC PCS7, SafeKit, a recognized solution on the SCADA market, 🔍 SafeKit High Availability Navigation Hub"
---

# High Availability for Siemens SIMATIC PCS 7: 2-Node SANless Cluster


<!-- BEGIN INSERT: insert-safekit-hyperv-solution-mirror-en lang="en" display="content" querystring="app=Siemens SIMATIC PCS7" -->
 

## SafeKit for Siemens SIMATIC PCS7: 2-Node SANless Hyper-V Cluster {#safekit-hyperv-tco}

**SafeKit for Siemens SIMATIC PCS7** delivers seamless **High Availability** while significantly reducing TCO (Total Cost of Ownership). By eliminating the hardware and operational overhead of traditional clustering, SafeKit offers a lean, 2-node HA alternative to complex virtualization stacks.
...
<img src="/wp-content/uploads/2026/01/safekit-vm-repli-720-720.jpg" width="512" alt="SafeKit High Availability Siemens SIMATIC PCS7 Cluster Architecture">

**SafeKit HA Cluster for Siemens SIMATIC PCS7**
...
## Table of Contents {#toc}
  ...
  * [How the SafeKit software simply implements a Siemens SIMATIC PCS7 high availability cluster without shared storage on a SAN?](<#safekit-hyperv-ha-cluster>)
  * [How the SafeKit Siemens SIMATIC PCS7 cluster works?](<#safekit-hyperv-ha-how-it-works>)
  * [SafeKit High Availability Limitations](<#safekit-ha-limitations>)
  * [Comparison: SafeKit for Siemens SIMATIC PCS7 vs. Native Hyper-V Clustering](<#safekit-hyperv-ha-comparison>)
  * [SafeKit video guide for Siemens SIMATIC PCS7](<#videoguide-safekit-hyperv-cluster>)
...
SafeKit brings high availability to Siemens SIMATIC PCS7 between two servers of any brand.
...
The following steps are described for one Siemens SIMATIC PCS7 virtual machine inside one mirror module. Each replicated virtual machine runs in an independent mirror module (with a maximum of 32 virtual machines) with a primary server that can be either the Hyper-V server 1 or the Hyper-V server 2.

### 1\. How does real-time replication work for Siemens SIMATIC PCS7?

Server 1 (PRIM) runs the Siemens SIMATIC PCS7 VM (virtual machine). SafeKit replicates in real time the VM files (virtual hard disk, VM configuration). Only changes made in the files are replicated across the network.


<img src="/wp-content/uploads/2023/02/safekit-prim-second-300.png" width="300" alt="File replication at byte level in a mirror Siemens SIMATIC PCS7 cluster">
...
When Server 1 fails, Server 2 takes over. SafeKit restarts the Siemens SIMATIC PCS7 VM on Server 2. Hyper-V finds the files replicated by SafeKit uptodate on Server 2.  
The VM continues to run on Server 2 by locally modifying its files that are no longer replicated to Server 1.


<img src="/wp-content/uploads/2023/02/safekit-stop-alone-300.png" width="300" alt="Failover of Siemens SIMATIC PCS7 in a mirror cluster">
...
<img src="/wp-content/uploads/2023/02/safekit-resynchro-alone-300.png" width="300" alt="Failback in a mirror Siemens SIMATIC PCS7 cluster">


Failback takes place without disturbing the Siemens SIMATIC PCS7 VM, which can continue running on Server 2.

### 4\. How is normal Siemens SIMATIC PCS7 operation restored?

After reintegration, the VM files are once again in mirror mode, as in step 1. The system is back in high-availability mode, with the Siemens SIMATIC PCS7 VM running on Server 2 and SafeKit replicating file updates to Server 1.


<img src="/wp-content/uploads/2023/02/safekit-second-prim-300.png" width="300" alt="Return to normal operation in a mirror Siemens SIMATIC PCS7 cluster">
...
### How to configure a SafeKit Siemens SIMATIC PCS7 Cluster? {#safekit-mirror-cluster-configure}


<img src="/wp-content/uploads/2024/03/19-module-hyperv-edit-config-1.png" width="694" alt="SafeKit Web Console: 2-node Siemens SIMATIC PCS7 cluster dashboard for VM replication and automatic failover.">
...
The **SafeKit web console** provides an intuitive interface to configure high availability of your critical applications. In just a few steps, you can configure a **SafeKit Siemens SIMATIC PCS7 cluster** to ensure business continuity:
  ...
  * Enter in `VM_NAME`, the name of the Siemens SIMATIC PCS7 virtual machine (VM1).
...
### How to monitor a SafeKit Siemens SIMATIC PCS7 cluster? {#safekit-mirror-cluster-monitoring}


<img src="/wp-content/uploads/2024/03/14-monitoring-prim-second.png" width="604" alt="SafeKit Web Console: Real-time monitoring of a 2-node mirror Siemens SIMATIC PCS7 cluster showing PRIM and SECOND states with active data replication.">
...
Feature | SafeKit for Siemens SIMATIC PCS7 | MS Failover Clustering | Hyper-V Replica  
---|---|---|---  
...
## Video Guide: Configuring a SafeKit HA cluster for Siemens SIMATIC PCS7 {#videoguide-safekit-hyperv-cluster}
...
<!-- END INSERT: insert-safekit-hyperv-solution-mirror-en lang="en" display="content" querystring="app=Siemens SIMATIC PCS7" -->
