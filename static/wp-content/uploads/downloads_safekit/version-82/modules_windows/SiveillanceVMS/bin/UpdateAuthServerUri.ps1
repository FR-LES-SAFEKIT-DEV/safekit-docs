...
function GetConfigurationPaths {
    $SafekitConfigPath = 'C:\safekit\modules\SiveillanceVMS\conf\userconfig.xml'
    $serverConfigPath = 'C:\ProgramData\Siemens\Siveillance VMS Management Server\ServerConfig.xml'
    $appSettingsPath = 'C:\Program Files\Siemens\Siveillance VMS Management Server\IIS\IDP\appsettings.json'
    ...
    while (-not (Test-Path $SafekitConfigPath)) {
        Write-Warning "Could not find userconfig.xml at $SafekitConfigPath"
        $SafekitConfigPath = Read-Host "Enter path to userconfig.xml"
    }
    return [psobject]@{
        ServerConfig = $serverConfigPath
        AppSettings = $appSettingsPath
        SafekitConfig = $SafekitConfigPath
    }
}

function GetVipFqdn ($skconfigpath) {
    $skconf=[xml](Get-Content -Path $skconfigpath -Raw)
    $name = $skconf.SelectSingleNode('/safe/service/vip/interface_list/interface/real_interface/virtual_addr').GetAttribute('addr')
    while ($null -eq $name) {
        Write-Information "No VIP was selected"
        $name = Read-Host "Enter a hostname or IP address matching the SafeKit virtual address"
    }
...
try {
    ...
    $uri = [uri]"http://$(GetVipFqdn($configs.SafekitConfig))/IDP"
    ...
    if ($authServerNode.InnerText.ToLower() -ne $uri.ToString().ToLower()) {
        ...
        $msSvc = Get-Service 'Siemens Siveillance VMS Management Server'
